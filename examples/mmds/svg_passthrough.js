...
readStdin((mmds) => {
  ...
  const pad = 20;
  const lines = [
    `<svg xmlns="http://www.w3.org/2000/svg" width="${b.width + pad * 2}" height="${b.height + pad * 2}">`,
  ];
  ...
  for (const n of mmds.nodes) {
    ...
    lines.push(
      `  <rect x="${x}" y="${y}" width="${n.size.width}" height="${n.size.height}" fill="none" stroke="black"/>`
    );
    lines.push(
      `  <text x="${n.position.x + pad}" y="${n.position.y + pad}" text-anchor="middle" dominant-baseline="central">${n.label}</text>`
    );
  }
