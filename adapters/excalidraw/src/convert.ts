...
): { strokeStyle: "solid" | "dotted"; strokeWidth: number } {
  switch (stroke) {
    case "dotted":
...
export function convert(mmds: MmdsDocument): ConvertResult {
  ...
  for (const n of mmds.nodes) {
    ...
    const txt = textElement(textId, left, top, w, h, n.label, NODE_FONT_SIZE, n.id);
  ...
  for (const e of mmds.edges) {
    ...
    if (e.label) {
      ...
      elements.push(
        textElement(labelId, labelX, labelY, 0, 0, e.label, EDGE_FONT_SIZE, arrowId),
      );
    } else {
      elements.push(arrowEl);
    }
  ...
  for (const el of elements) {
    ...
    if (nodeBound.has(id)) {
      el.boundElements = nodeBound.get(id)!;
    }
  }
