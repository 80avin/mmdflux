# Orthogonal Routing Promotion Decision Record

Use this decision record when promoting orthogonal routing from preview to the default behavior.
...
- [x] Why graph-family promotion is deferred:
  - Class diagram DAGRE SVG path still uses legacy direct render path in `ClassInstance` and does not mirror flowchart orthogonal-preview call-site routing.
  ...
  - Flowchart has explicit orthogonal-vs-legacy parity/rollback harnesses; class currently has snapshot compliance coverage but no equivalent edge-routing parity gate.
...
- Fixture matrix status:
  - Pass (`SVG` + `Text`) for fan-in overflow toggle replay (`on`/`off`) across:
    ...
    - `git_workflow.mmd`
- Decision:
  - Keep fan-in overflow/backward-channel behavior **always enabled** for orthogonal preview.
- Evidence gates:
...
- Gate reproducibility is anchored in:
  - `scripts/tests/08-orthogonal-vs-full-svg-diff-sweep.sh`
  - `docs/orthogonal_feedback_baseline.tsv`
...
- Orthogonal preview now has a single canonical path for long-skip edges (no on/off matrix).
...
This is the decision record for known output differences when orthogonal routing is promoted.

| Area                                                                                                                                                                                                                                   | Classification       | Decision             | Notes                                                                                                                                                                       |
| -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------- | -------------------- | --------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Flowchart SVG straight parity-classification subset (`simple.mmd`, `chain.mmd`, `simple_cycle.mmd`, `decision.mmd`, `fan_out.mmd`, `left_right.mmd`, `subgraph_direction_cross_boundary.mmd`, `multi_subgraph_direction_override.mmd`) | accepted-improvement | Accept for Release N | Orthogonal-preview deltas are explicitly classified and test-enforced in `svg_orthogonal_preview_parity_fixture_subset_matches_expected_classification`.                    |
| Flowchart text label-revalidation fixture parity (`labeled_edges.mmd`, `inline_label_flowchart.mmd`)                                                                                                                                   | must-match           | Must match           | `text_label_revalidation_fixtures_match_between_orthogonal_preview_and_full_compute_modes` enforces edge-routing parity for text output across label-revalidation fixtures. |
...
- `plans/archive/0075-orthogonal-routing-unification/findings/discovery-orthogonal-preview-svg-linear-core-parity-delta-simple-cycle.md`
- `plans/archive/0075-orthogonal-routing-unification/findings/note-orthogonal-preview-backward-edge-fallback-uses-existing-hints.md`
- `tests/svg_snapshots.rs`:
  - `svg_orthogonal_preview_parity_fixture_subset_matches_expected_classification`
  - `orthogonal_preview_svg_output_is_deterministic_for_fixture_subset`
  - `svg_full_compute_override_matches_legacy_straight_core_subset`
- `tests/mmds_json.rs`:
  - `orthogonal_preview_mmds_routed_output_is_deterministic_for_fixture_subset`
  - `routed_mmds_defaults_to_full_path_detail`
- `tests/integration.rs`:
  - `text_label_revalidation_fixtures_match_between_orthogonal_preview_and_full_compute_modes`
...
- [ ] Create follow-up implementation plan: **class parity/rollback harness**
  - Add class orthogonal-vs-legacy edge-routing parity tests equivalent in intent to flowchart gates.
- [ ] Create follow-up implementation plan: **graph-family default promotion**
  - Promote orthogonal routing default across class + flowchart only after the above two plans are complete.
...
- [x] Determinism confirmed for orthogonal mode (same input, same output bytes):
  - SVG (`--edge-routing orthogonal-preview`)
  - MMDS routed (`--edge-routing orthogonal-preview`)
...
- [x] `simple_cycle.mmd` straight-SVG delta in orthogonal preview is accepted (or fixed):
  - `plans/archive/0075-orthogonal-routing-unification/findings/discovery-orthogonal-preview-svg-linear-core-parity-delta-simple-cycle.md`
- [x] Backward-edge fallback is accepted (or replaced with full orthogonal behavior):
  - `plans/archive/0075-orthogonal-routing-unification/findings/note-orthogonal-preview-backward-edge-fallback-uses-existing-hints.md`
...
| Label-revalidation text parity                      | `labeled_edges.mmd`, `inline_label_flowchart.mmd`                                                                                                            | `must-match` between `orthogonal-preview` and `full-compute` text output | `text_label_revalidation_fixtures_match_between_orthogonal_preview_and_full_compute_modes`                                                                                                                                                                                        |
...
- [ ] Change default routing selection to orthogonal mode for intended scope.
- [ ] Keep CLI override behavior intact:
  - `--edge-routing orthogonal-preview`
...
```text
Routing default changed: orthogonal routing is now the default for <scope>.
