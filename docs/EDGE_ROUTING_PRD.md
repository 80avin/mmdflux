...
| R-BACK-9 | A side-channel backward edge must have exactly 4 bends: depart source face → turn into channel → straight channel run → turn toward target → enter target face | P1 | The rectangular U-shape is the minimum-bend solution for side-channel routing. Any additional bends (jogs, asymmetric dips, outward-then-inward excursions) are waste that degrades readability per Purchase (1997) and Ware et al (2002). Observed in `git_workflow.mmd` LR rendering where the backward edge has 6 bends due to an asymmetric downward dip near the target, and in the TD rendering where an outward-then-inward jog adds 2 unnecessary bends before the channel run. |
